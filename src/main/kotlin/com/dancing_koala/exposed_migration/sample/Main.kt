...
import com.dancing_koala.exposed_migration.lib.Migration
import com.dancing_koala.exposed_migration.lib.Migrator
...
internal object Migrate2To3 : Migration(2, 3) {
    private val sql = """
...
internal fun getVersions() = transaction {
    MigrationEntity.find { (MigrationsTable.successful eq true) }.map { it.version }
}

fun main() {
    // Delete DB if already exists
    File("src/main/resources/test.mv.db").apply { if (exists()) delete() }

    val dbFile = File("src/main/resources/test")
